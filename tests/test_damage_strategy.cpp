...
TEST_F(DamageStrategyTest, ElementalDamageCritRate) {
    ...
    // 12%暴击率，100次应该有约12次暴击
    EXPECT_GT(crit_count, 5); // 至少5次
    EXPECT_LT(crit_count, 25); // 最多25次
}
...
TEST_F(DamageStrategyTest, ElementalDamageHalfDefense) {
    ...
    int no_defense = strategy.calculateDamage(20, 0, isCrit);
    int with_defense = strategy.calculateDamage(20, 20, isCrit);
    
    // 伤害差值应该大约是防御的一半
    int diff = no_defense - with_defense;
    EXPECT_LE(diff, 15); // 不应该超过15（20/2 + 波动）
}
...
TEST_F(DamageStrategyTest, HighAttack) {
    ...
    EXPECT_LE(damage, 2000); // 但不应该超过2倍（暴击）
}
